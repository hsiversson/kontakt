	...
	uint32& aOutIndexStride)
{
	...
	for (const Gfx_Meshlet& meshlet : aMeshlets)
	{
		const Gfx_PackedPrimitiveTriangle* primitives = &aPrimitives[meshlet.mPrimitiveOffset];
		...
		{
			const Gfx_PackedPrimitiveTriangle& triangle = primitives[i];
			const SC_Vector3ui triIndices = triangle.Unpack();
...
static bool AddCandidateToMeshlet(uint32 aMaxVertices, uint32 aMaxPrimitives, InlineMeshlet<IndexType>& aMeshlet, IndexType(&aTriangle)[3])
{
	...
	Gfx_PackedPrimitiveTriangle& primitiveTriangle = aMeshlet.mPrimitiveIndices.Add();
...
static float ComputeScore(const InlineMeshlet<IndexType>& aMeshlet, const SC_Vector4& aSphere, const SC_Vector& aNormal, IndexType(&aTriIndices)[3], SC_Vector* aTriVerts)
{
	...
	float r = aSphere.w;
	float r2 = r * r;
	float locScore = SC_Log2(maxSq / r2 + 1);
	...
	SC_Array<InlineMeshlet<IndexType>>& aOutMeshlets)
{
	...
	BuildMeshletAdjacencyList(aIndexData, aNumIndices, aVertexData, aNumVertices, aVertexLayout, adjacencyList);
...
template<class IndexType>
static void BuildMeshletAdjacencyList(
	const IndexType* aIndexData,
	const uint32 aNumIndices,
	const uint8* aVertexData,
	const uint32 aNumVertices,
...
bool Gfx_GenerateMeshlets(
	...
	bool aUse16BitIndices, const uint32 aNumIndices, SC_Array<Gfx_Meshlet>& aOutMeshlets, SC_Array<Gfx_PackedPrimitiveTriangle>& aOutPrimitives, SC_Array<uint8>& aOutVertexIndices)
{
	if (aUse16BitIndices)
	{
		const uint16* indexData = reinterpret_cast<const uint16*>(aIndexData);
		return GenerateMeshlets(aMaxVertices, aMaxPrimitives, aVertexData, aVertexLayout, aNumVertices, indexData, aNumIndices, aOutMeshlets, aOutPrimitives, aOutVertexIndices);
	}
	else
	{
