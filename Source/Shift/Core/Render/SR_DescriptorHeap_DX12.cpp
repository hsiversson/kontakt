...
SR_DescriptorHeap_DX12::SR_DescriptorHeap_DX12(uint32 aNumDescriptors, const SR_DescriptorHeapType& aType, bool aShaderVisible /*= false*/, const char* aDebugName /*= nullptr*/)
	: mMaxNumDescriptors(aNumDescriptors)
{
	...
	if (aDebugName)
	{
		...
		SC_UTF::ToUTF16(debugName, aDebugName, (uint32)strlen(aDebugName));
		mD3D12DescriptorHeap->SetName(debugName.c_str());
	}
}
