...
bool SR_RootSignature_DX12::Init()
{
	...
	for (const SR_StaticSamplerProperties& staticSamplerProps : mProperties.mStaticSamplers)
	{
		...
		staticSampler.Filter = SR_ConvertFilter_DX12(sampler.mMinFilter, sampler.mMagFilter, sampler.mMipFilter, sampler.mComparison, sampler.mMaxAnisotropy);
		staticSampler.MaxAnisotropy = 16;
		staticSampler.AddressU = SR_ConvertWrapMode_DX12(sampler.mWrapX);
		staticSampler.AddressV = SR_ConvertWrapMode_DX12(sampler.mWrapY);
		staticSampler.AddressW = SR_ConvertWrapMode_DX12(sampler.mWrapZ);
		staticSampler.ComparisonFunc = SR_ConvertComparisonFunc_DX12(sampler.mComparison);
	...
	{
		hr = mD3D12RootSignature->SetName(SC_UTF::ToUTF16(mProperties.mDebugName).c_str());
		if (!SR_VerifyHRESULT(hr))
			return false;
	}
	return true;
