...
public:
	void Init(const SR_RasterizerStateProperties& aProperties)
	{
		...
		mDesc.CullMode = SR_ConvertCullMode_DX12(aProperties.mCullMode);
...
private:
	...
	uint32 _pad; // used to hide warning thrown because of padding.
...
public:
	void Init(const SR_BlendStateProperties& aProperties)
	{
		...
		for (uint32 RtIndex = 0; RtIndex < SR_MAX_RENDER_TARGETS; ++RtIndex)
		{
			...
			rtBlendDescDst.BlendOp = SR_ConvertBlendFunc_DX12(rtBlendProps.mBlendFunc);
			rtBlendDescDst.BlendOpAlpha = SR_ConvertBlendFunc_DX12(rtBlendProps.mBlendFuncAlpha);
			rtBlendDescDst.SrcBlend = SR_ConvertBlendMode_DX12(rtBlendProps.mSrcBlend);
			rtBlendDescDst.SrcBlendAlpha = SR_ConvertBlendMode_DX12(rtBlendProps.mSrcBlendAlpha);
			rtBlendDescDst.DestBlend = SR_ConvertBlendMode_DX12(rtBlendProps.mDstBlend);
			rtBlendDescDst.DestBlendAlpha = SR_ConvertBlendMode_DX12(rtBlendProps.mDstBlendAlpha);
			rtBlendDescDst.RenderTargetWriteMask = SR_ConvertColorWriteMask_DX12(rtBlendProps.mWriteMask);
...
public:
	void Init(const SR_DepthStencilStateProperties& aProperties)
	{
		...
		mDesc.DepthFunc = SR_ConvertComparisonFunc_DX12(aProperties.mDepthComparisonFunc);
		...
		mDesc.FrontFace.StencilFailOp = SR_ConvertStencilOperator_DX12(aProperties.mFrontFace.mFail);
		mDesc.FrontFace.StencilDepthFailOp = SR_ConvertStencilOperator_DX12(aProperties.mFrontFace.mDepthFail);
		mDesc.FrontFace.StencilPassOp = SR_ConvertStencilOperator_DX12(aProperties.mFrontFace.mPass);
		mDesc.FrontFace.StencilFunc = SR_ConvertComparisonFunc_DX12(aProperties.mFrontFace.mComparisonFunc);
		mDesc.BackFace.StencilFailOp = SR_ConvertStencilOperator_DX12(aProperties.mBackFace.mFail);
		mDesc.BackFace.StencilDepthFailOp = SR_ConvertStencilOperator_DX12(aProperties.mBackFace.mDepthFail);
		mDesc.BackFace.StencilPassOp = SR_ConvertStencilOperator_DX12(aProperties.mBackFace.mPass);
		mDesc.BackFace.StencilFunc = SR_ConvertComparisonFunc_DX12(aProperties.mBackFace.mComparisonFunc);
	}
};
...
public:
	...
	void Init(const SR_RenderTargetFormats& aRTVFormats)
	{
		...
		for (uint32 i = 0; i < aRTVFormats.mNumColorFormats; ++i)
			mDesc.RTFormats[i] = SR_ConvertFormat_DX12(aRTVFormats.mColorFormats[i]);
	}
};
...
public:
	...
	{
		mFormat = SR_ConvertFormat_DX12(aFormat);
	}
};
